<?php session_start();
setcookie("sessid", session_id(), time()+3600*24);
//if(!isset($_SESSION["authResult"])){$_SESSION["authResult"]="";}
?>
...
<?
echo 'sid = "'.session_id().'";';
echo 'token = "'.$_SESSION["authResult"].'";';
?>
  ...
  function loginFinishedCallback(authResult) {
    if (authResult) {
      if (authResult['error'] == undefined){
        ...
        //storetoken(authResult);					// Store the token in session
...
var infowindow = new google.maps.InfoWindow(
  ...
  function initialise() {
	...
	windows = [];
	loadMarkers();
<? echo 'loginFinishedCallback('.$_SESSION["authResult"].');';
?>
  }
...
downloadUrl("get.php", function(data) {
  ...
  function positionSuccess(position) {
    ...
    (new google.maps.Geocoder()).geocode({latLng: latLng}, function(resp) {
		  ...
		  if (resp[0]) {
			...
			document.getElementById('texte').innerHTML = place;
	  });
  }
   ...
   */
    ...
    function storetoken(authRes) {
	 
	 var donnees = authRes.access_token;
	 console.log(donnees);
    $.ajax({
	type: "POST",
	url: "setsession.php",
...
<body onload="initialise()">
  ...
  <div id="signin-button" class="show">
     <div class="g-signin" data-callback="loginFinishedCallback"
      ...
      data-height="short"
      data-cookiepolicy="http://rlier.fr"
      >
    </div></div>
</body>
</html>
